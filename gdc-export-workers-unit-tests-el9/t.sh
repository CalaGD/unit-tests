...
stop_and_clean() {
  if podman pod exists test-gdc-export-workers; then
    podman pod stop test-gdc-export-workers
    podman pod rm test-gdc-export-workers
  fi
}
...
build_and_create() {
  ...
  podman pod create test-gdc-export-workers
}
...
run_tests() {
  ...
  then
    ...
    podman run -it --pod test-gdc-export-workers --entrypoint /usr/bin/bash tests
  else
	if [ ! -z "$TEST_PATH" ];
	then
      		# Run a single test file
      		podman run -it --pod test-gdc-export-workers tests $TEST_PATH
     	else
		# Run all tests
		podman run -it --pod test-gdc-export-workers tests
	fi
  fi
}
